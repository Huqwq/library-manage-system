...
void lianjie()
{
	...
	sock = mysql_real_connect(&mysql, "localhost", "root", "020866", "book", 3306, NULL, 0);
	if (sock == NULL)//连接数据库
	{
		printf("\n连接数据库时发生错误!\n");
...
void showreader()
{
	...
	int num;        //用来存放结果集列数
	int i;
	mysql_real_connect(&mysql, "localhost", "root", "020866", "book", 3306, NULL, 0);
	mysql_query(&mysql, "set names gbk");
	mysql_query(&mysql, "select * from 借阅者信息表");
...
void showjieyue()
{
	...
	mysql_real_connect(&mysql, "localhost", "root", "020866", "book", 3306, NULL, 0);
	mysql_query(&mysql, "set names gbk");
	mysql_query(&mysql, "select * from 借阅信息表");
	result = mysql_store_result(&mysql);    //将查询到的结果集储存到result中
	num = mysql_num_fields(result);        //将结果集列数存放到num中
	cout << "编号       书名                  借书日期        还书日期         借书人" << endl;
	while ((row = mysql_fetch_row(result)))  //遇到最后一行，则中止循环
	{
		for (i = 0; i < num; i++)         //利用for循环，输出该行的每一列
		{
...
void insertreader()
{
	...
	int  xue;
	mysql_real_connect(&mysql, "localhost", "root", "020866", "借阅者信息表", 3306, NULL, 0);//连接数据库
...
void insertjieyue()
{
	...
	int  bianhao;
	mysql_real_connect(&mysql, "localhost", "root", "020866", "借阅信息表", 3306, NULL, 0);//连接数据库
	mysql_query(&mysql, "SET NAMES GBK");//可以中文
	cout << "编号    书名                              借书日期            还书日期           借书人 " << endl;
